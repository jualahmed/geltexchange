...
function checkAvailability() {
	var url = $("#url").val();
	var data_url = url + "requests/check_availability.php";
...
function bit_calculator_fromrecive(){
	var currency_from = $("#bit_currency_from").val();
	var currency_to = $("#bit_currency_to").val();
	var rate_from = $("#bit_rate_from").val();
	var rate_to = $("#bit_rate_to").val();
	var amount_send = $("#bit_amount_receive").val();
	var amount_send1=amount_send;
	if(isNaN(amount_send)) {
		var data = '0';
	} else {
		if(isCrypto(currency_from) && isCrypto(currency_to)) {
	...
	$("#bit_amount_receive2").val(amount_send1);
}
...
jQuery(document).ready(function($) {
	...
	var scroll_to = $('.scrollbar-outer1').offset().top + $('.scrollbar-outer1').height();
	$('#clickforscroll').click(function(event) {
		event.preventDefault();
	    $('.scrollbar-outer1').animate({
	        scrollTop: scroll_to
	    }, 2000);
	});

	var scroll_tos = $('.scrollbaroutertest').offset().top + $('.scrollbaroutertest').height();
	$('#scrollbaroutertestddd').click(function(eventd) {
		eventd.preventDefault();
	    $('.scrollbaroutertest').animate({
	        scrollTop: scroll_tos
	    }, 2000);
	});
});
...
setInterval(function(){
	var url = $("#url").val();	
	var data_url = url + "requests/refreshdata.php";
	$.ajax({
		type: "POST",
		url: data_url,
		success: function (data) {
